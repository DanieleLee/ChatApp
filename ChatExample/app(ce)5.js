...
var socketio=require('socket.io');
 ...
 // cors 사용- 클라이언트에서 ajax로 요청하면 CORS지원
...
io.sockets.on('connection', function(socket) {
    ...
    socket.on('message',function(message){
        ...
        // command 속성으로 일대일과 그룹 채팅 구별
         if(message.command=='chat'){
            // 일대일 채팅 대상에게 메세지 전달
            if(login_ids[message.recepient]){
                io.sockets.connected[login_ids[message.recepient]].emit('message',message);
                // 클라이언트가 로그인하지않으면 login_ids객체에 로그인 ID가 없을수도있다.
                
                // 응답 메세지 전송
                sendResponse(socket,'message','200','메세지를 전송했습니다!');
            }else{
                sendResponse(socket,'login','404','상대방의 로그인 ID를 찾을수없다!!');
            }
            ...
            io.sockets.in(message.recepient).emit('message',message);
             ...
             //응답 메세지 전송
             sendResponse(socket,'message','200','방['+message.recepient+']의 모든사용자들에게 메세지를 전송하였음');
         }
         }
    });
    ...
    socket.on('login',function(login){
        ...
        console.log('접속한 소켓ID:'+socket.id);
        login_ids[login.id]=socket.id;
        socket.login_id=login.id;
        
        console.log('접속한 클라이언트 ID개수: %d',Object.keys(login_ids).length);
        
        for(var key in login_ids){
            console.log('login Attri>>'+key+',value>>'+login_ids[key]);
        }
    ...
    //'logout' 이벤트를 받았을때의 처리
    socket.on('logout',function(logout){
        console.log('로그아웃 이벤트를 요청받음!!');
        
        for(var key in login_ids){
            
            if(key==logout.id){
                 login_ids[key]=undefined;
               
            }
            console.log('attri>>'+key+',val>>'+login_ids[key]);
        }
    ...
    socket.on('room',function(room){
        ...
        if(room.command=='create'){
            if(io.sockets.adapter.rooms[room.roomId]){// 방이 이미 만들어져 있는경우에
            ...
            if(io.sockets.adapter.rooms[room.roomId]){// 방이 만들어져 있을때
...
function getRoomList(){
    ...
    Object.keys(io.sockets.adapter.rooms).forEach(function(roomId){ // 각방에대해 처리
